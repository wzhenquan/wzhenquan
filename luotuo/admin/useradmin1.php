...
if($_SESSION['author']==0)exit();

mysqli_query($GLOBALS['conn'],"alter table chzb_serialnum add column vip int DEFAULT 0;");
function genName(){
...
if(isset($_POST['submitsn2'])){
	...
	if($row=mysqli_fetch_array($result)){
		...
		if($row=mysqli_fetch_array($result)){
			...
			$sql="INSERT into chzb_serialnum (sn,exp,author,createtime,marks,vip) values($sn,$exp,'$user',$nowtime,'$marks',$vip)";
...
if(isset($_POST['submitsn'])){
	...
	for ($i=0; $i <$snCount ; $i++) { 
		...
		$nowtime=time();
		$snall=$snall.$sn;
		$sql="INSERT into chzb_serialnum (sn,exp,author,createtime,marks,vip) values($sn,$exp,'$user',$nowtime,'$marks',$isvip)";
		mysqli_query($GLOBALS['conn'],$sql);
	}
...
<form method="post">
	...
	<input type="checkbox" name='vip'>开启隐藏节目
...
<table border="1" bordercolor="#a0c6e5" style="border-collapse:collapse;">
	<tr><td colspan=7><b>账号列表</b>	
			...
			&nbsp;&nbsp;&nbsp;&nbsp;
			<form method="POST" id="recCounts">每页
	...
	<tr><td width=150><a href="?order=sn">账号<?php if($order=='sn') echo'↓'?></a></td>
		...
		<td width=200><a href="?order=marks">备注<?php if($order=='marks') echo'↓'?></a></td></tr>
<form method="POST">
<?php
$recStart=$recCounts*($page-1);
if($user=='admin'){
	$sql="select sn,regtime,exp,author,createtime,enable,marks,vip from chzb_serialnum where 1=1 $searchparam order by $order desc limit $recStart,$recCounts";
}else{
	$sql="select sn,regtime,exp,author,createtime,enable,marks,vip from chzb_serialnum where author='$user' $searchparam order by $order desc limit $recStart,$recCounts";
}
...
while($row=mysqli_fetch_array($result)){
	...
	echo "<tr><td><input type='checkbox' value='$sn' name='sn[]'><font color='$fontcolor'>".$sn."</font></td><td>".$regtime."</td><td>".$days. "</td><td>".$author. "</td><td>" .$isactived."</td><td>$createtime</td><td>$marks</td></tr>";
}
?>
<tr><td colspan="7">
